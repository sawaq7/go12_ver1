...
///                           　　　　　　　　　　　
///   ストリング型のラインデータを構造体にセット（Water2_Temp)（水路ファイル・ストレッジ）
///                          　　　　　　　　　　　

func Water2_temp( w http.ResponseWriter, water2_temp_string string )  ( type4.Water2_Temp ) {

//     IN         w              : レスポンスライター
//     IN   water2_temp_string   : 水路ファイルのラインデータ（string型）
//     OUT  water2_temp_struct   : 水路ファイルのラインデータ（struct型・Water2_Temp）

//    fmt.Fprintf( w, "struct_set.water2_temp start \n" )  // デバック
//    fmt.Fprintf(w, "struct_set.water2_temp : water2_temp_string %s\n", water2_temp_string )  // デバック

    var water2_temp_struct type4.Water2_Temp                 //   Water2_Temp　のワークエリアを確保

    str := strings.Fields(water2_temp_string)  // ストリング型のラインデータを分割
...
//          fmt.Printf("struct_set: len(str2) %v\n", len(str2) )
          ...
          water2_temp_struct.Id ,_ =strconv.ParseInt(str2[1],10 ,64)   // 文字列をint64に変換
